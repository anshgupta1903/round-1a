...
try:
    model = joblib.load('../scripts/model/heading_model.pkl')
    label_mapping = joblib.load('../scripts/model/label_mapping.pkl')
...
def process_pdf(pdf_path, ml_output_path=None):
    ...
    from pipeline.pipeline import preprocess_for_model
    ...
    for line in all_lines:
        if line['id'] not in all_candidates:
            text = line['text']
            is_stylistically_distinct = (line['size'] > body_style[0]) or (line['bold'] and not body_style[1])
